...
const Score = (() => {
  ...
  function calculateScore(han, isYakuman, situation) {
    ...
    const isTsumo = situation.isTsumo;
  ...
  function formatScore(scoreResult) {
    ...
    if (scoreResult.isTsumo) {
      const ko = scoreResult.payments.ko.toLocaleString();
      const oya = scoreResult.payments.oya.toLocaleString();
      return `${scoreResult.rank} ${total}点 (${ko}/${oya}点)`;
    } else {
